...
BOLD.recurring_orders.setupEventListeners = function(){
  if(BOLD.common && BOLD.common.eventEmitter){
    ...
    if(window.jQuery && typeof jQuery().on === 'function' && !BOLD.recurring_orders.jQuerySetup){
      ...
      jQuery(document).on('click', '[name="checkout"]:not(.bold_clone):not([action*="/tools/checkout"]),[href*="/checkout"]:not(.bold_clone):not([action*="/tools/checkout"])', BOLD.helpers.triggerCheckoutEvent);
...
BOLD.recurring_orders.redirectToRecurringCheckout = function(form){
  ...
  if((current_mode == BOLD.recurring_orders.modes.mixed_cart && window.mixed_cart) || (current_mode == BOLD.recurring_orders.modes.recurring_cart && form.recurring_radio_btn && form.recurring_radio_btn.value == 1)){
    var url = BOLD.recurring_orders.base_checkout_url + '/s/' + BOLD.common.Shopify.shop.permanent_domain.replace('.myshopify.com','') + '/checkout/recurring' + (current_mode == BOLD.recurring_orders.modes.mixed_cart ? '/' + BOLD.common.Shopify.cart.token : '_full_cart') + '?shop_url=' + BOLD.common.Shopify.shop.permanent_domain + (typeof google_analytics_get_param_string === 'function' ? google_analytics_get_param_string() : '');
    ...
    cart_field.setAttribute('value',encodeURIComponent(JSON.stringify(BOLD.rawCart || BOLD.common.Shopify.cart)));
...
BOLD.recurring_orders.updateRecurringWidget = function(data){
  //Make sure that we're in single-product or mixed-cart mode
  var modes = BOLD.recurring_orders.modes || {};
  if(!(modes.current_mode && (modes.current_mode == modes.single_product || modes.current_mode == modes.mixed_cart))){
    return;
  }
